...
# -*- coding: utf-8 -*-
"""
자동 성능 평가 및 반복 훈련 시스템
Korean QA RAG Auto Training with Performance Evaluation
...
class AutoTrainingEvaluator:
    ...
    def __init__(self, 
        ...
        logger.info(f"자동 훈련 시스템 초기화 완료")
...
답변:"""
    ...
    def run_training_epoch(self, iteration: int) -> str:
        ...
        logger.info(f"=== 반복 {iteration} 훈련 시작 ===")
        ...
        logger.info(f"=== 반복 {iteration} 훈련 완료 ===")
    ...
    def evaluate_model(self, model_path: str) -> Dict[str, float]:
        ...
        try:
            ...
            logger.info(f"테스트 데이터 {len(self.test_data)}개로 평가 중...")
            
            for i, sample in enumerate(self.test_data):
                if i % 50 == 0:
                    logger.info(f"평가 진행률: {i}/{len(self.test_data)}")
    ...
    def run_auto_training(self):
        """자동 훈련 실행"""
        logger.info("🚀 자동 훈련 시스템 시작!")
        
        os.makedirs(self.base_output_dir, exist_ok=True)
        
        for iteration in range(1, self.max_iterations + 1):
            logger.info(f"\n{'='*60}")
            logger.info(f"반복 {iteration}/{self.max_iterations} 시작")
            logger.info(f"{'='*60}")
            ...
            if self.check_target_achieved(scores):
                logger.info(f"🎯 목표 점수 달성! 반복 {iteration}에서 완료")
                break
            
            # 훈련 기록 저장
            self.save_training_history()
            
            logger.info(f"반복 {iteration} 완료. 다음 훈련 준비...")
            time.sleep(2)  # 잠시 대기
        
        # 최종 결과 요약
        logger.info(f"\n{'='*60}")
        logger.info("🏁 자동 훈련 완료!")
        logger.info(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description="자동 성능 평가 및 반복 훈련")
    ...
    # 목표 점수 설정
    parser.add_argument("--target_exact_match", type=float, default=0.75,
                       help="목표 정확도 (Exact Match)")
    parser.add_argument("--target_f1_score", type=float, default=0.80,
                       help="목표 F1 점수")
    parser.add_argument("--target_bleu_score", type=float, default=0.70,
                       help="목표 BLEU 점수")
    ...
    # 자동 훈련 시스템 실행
    auto_trainer = AutoTrainingEvaluator(
