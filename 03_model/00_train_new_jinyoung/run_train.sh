...
# 8비트 양자화 + LoRA 훈련 스크립트

echo "========================================="
echo "Qwen3-8B 한국어 QA RAG 파인튜닝 시작"
echo "8비트 양자화 + LoRA 어댑터 훈련"
echo "Korean QA RAG Fine-tuning with 8-bit + LoRA"
echo "========================================="
...
export WANDB_DISABLED=true  # WandB 비활성화
...
echo "GPU: RTX A6000 48GB x2 (멀티GPU 자동 매핑)"
echo "LoRA 설정: rank=16, alpha=32, dropout=0.1"
echo "에포크: 3"
echo "배치 크기: 64 (per_device=4 × 2 GPU × gradient_accumulation=8)"
...
# 훈련 실행 (8비트 양자화 + LoRA는 단일 GPU에서 실행)
echo "=== 훈련 시작 ==="
uv run python train_jinyoung.py \
    --model_name_or_path "Qwen/Qwen3-8B" \
    --train_data_path "${TRAIN_DATA}" \
    --eval_data_path "${EVAL_DATA}" \
    --output_dir "./results/qwen3-8b-korean-qa-rag-lora" \
    --max_seq_length 2048 \
    --num_train_epochs 3 \
...
if [ $? -eq 0 ]; then
    ...
    echo "========================================="
    echo "✓ 훈련 완료 시간: $(date)"
    echo "✓ 8비트 양자화 + LoRA 훈련 완료"
    echo "✓ LoRA 어댑터 저장 위치: ./results/qwen3-8b-korean-qa-rag-lora"
    echo ""
    echo "저장된 모델 크기 확인:"
    du -sh ./results/qwen3-8b-korean-qa-rag-lora/
    echo ""
    echo "저장된 파일 목록:"
    ls -la ./results/qwen3-8b-korean-qa-rag-lora/
    echo ""
    ...
    echo "========================================="
    echo "✗ 훈련 실패!"
    echo "✗ 로그를 확인하세요: ./results/qwen3-8b-korean-qa-rag-lora/logs"
