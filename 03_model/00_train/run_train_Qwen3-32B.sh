...
# Qwen3-32B 4bit LoRA 한국어 QA RAG 파인튜닝 스크립트
# Korean QA RAG Fine-tuning Script for Qwen3-32B with 4-bit LoRA

echo "========================================="
echo "한국어 QA RAG 파인튜닝 시작 (Qwen3-32B 4bit LoRA) (Loss 0.05 이하까지)"
echo "Korean QA RAG Fine-tuning Started (Qwen3-32B 4bit LoRA) (Until Loss < 0.05)"
echo "========================================="
...
echo "시퀀스 길이: 2048 (32B 모델에 맞게 조정)"
echo "양자화: 4bit (메모리 최적화)"
echo "메모리 최적화: gradient_checkpointing 활성화"
echo "========================================="

# Qwen3-32B 4bit LoRA로 Loss 0.05 이하까지 훈련하기 위한 파라미터 (2 GPU 분산)
uv run python -m torch.distributed.launch --nproc_per_node=2 --use_env train.py \
    ...
    --output_dir ./results/qwen3-32b-4bit-lora-korean-qa-rag \
...
echo "========================================="
echo "훈련 완료 시간: $(date)"
echo "32B 모델 4bit LoRA + RTX A6000 2장 분산 훈련으로 메모리 효율성 및 훈련 속도가 최적화되었습니다"
echo "배치 사이즈: 16 (per_device_train_batch_size=1 × 2 GPU × gradient_accumulation_steps=8)"
echo "그라디언트 누적 단계: 8 (총 유효 배치 사이즈: 16)"
echo "시퀀스 길이: 2048 (32B 모델에 맞게 조정)"
