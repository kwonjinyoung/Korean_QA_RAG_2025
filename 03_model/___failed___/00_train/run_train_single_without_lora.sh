...
# Qwen3-8B 4bit 한국어 QA RAG 파인튜닝 스크립트 (LoRA 없음)
# Korean QA RAG Fine-tuning Script for Qwen3-8B with 4-bit (without LoRA)

echo "========================================="
echo "한국어 QA RAG 파인튜닝 시작 (Qwen3-8B 4bit, LoRA 없음) (Loss 0.05 이하까지)"
echo "Korean QA RAG Fine-tuning Started (Qwen3-8B 4bit, without LoRA) (Until Loss < 0.05)"
echo "========================================="
...
# Qwen3-8B 4bit로 Loss 0.05 이하까지 훈련하기 위한 파라미터 (1 GPU, LoRA 없음)
...
echo "========================================="
echo "훈련 완료 시간: $(date)"
echo "8B 모델 4bit + RTX 4090 1장 훈련으로 메모리 효율성 및 훈련 속도가 최적화되었습니다 (LoRA 없음)"
echo "배치 사이즈: 1 (per_device_train_batch_size=1 × 1 GPU)"
echo "그라디언트 누적 단계: 32 (총 유효 배치 사이즈: 32)"
echo "시퀀스 길이: 4096"
echo "메모리 최적화: gradient_checkpointing 활성화, 4비트 양자화 적용"
